...
trait WikipediaApi {
  ...
  implicit class StringObservableOps(obs: Observable[String]) {
    ...
    /** Given a stream of search terms, returns a stream of search terms with spaces replaced by underscores.
      ...
      */
    def sanitized: Observable[String] = obs.map(it => it.replaceAll(" ", "_"))
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
    ...
    //        case OnNext(value) => Success(value)
    //        case OnError(ex) => Failure(ex)
    //      } take(obs.size.toBlockingObservable.single)
    ...
    //            case OnError(ex) => observer.onNext(Failure(ex))
    ...
    def timedOut(totalSec: Long): Observable[T] = {
      @volatile var notTimedOut = true
      future {
        Thread.sleep(totalSec * 1000)
        notTimedOut = false
      }
      obs.takeWhile(it => notTimedOut)
    }
...
//      })
    ...
    def recovered: Observable[Try[T]] = obs.materialize.flatMap {
      case OnNext(value) => Observable(Success(value))
      case OnError(error) => Observable(Failure(error))
      case OnCompleted(_) => Observable()
    }
