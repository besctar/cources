...
trait NodeScala {
  ...
  private def respond(exchange: Exchange, token: CancellationToken, response: Response): Unit = {
    val timeoutTokenSource = CancellationTokenSource()
    Future.delay(10 second).onComplete(it => timeoutTokenSource.unsubscribe)
    val timeoutToken = timeoutTokenSource.cancellationToken

    try {
      for (line <- response) {
        if (token.isCancelled || timeoutToken.isCancelled) {
    ...
    //    response.foreach {
    //      line =>
    //        if (token.nonCancelled && timeoutToken.nonCancelled) {
    //          exchange.write(line)
    //        } else {
    //          throw new CancellationException
    //        }
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    //    def loop(body:Future => Unit):  Unit = body andThen loop(body)
...
object NodeScala {
  ...
  trait Listener {
    ...
    /** Given a relative path:
      ...
      * 2) installs an asynchronous request handler using `createContext` that completes the promise with a request when it arrives and then deregisters itself using `removeContext`
